...
class vbro_images
{
    static public function init()
    {
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . 'vbro_images';
    ...
    {
        global $wpdb;
        $query = 'SELECT * FROM ' . $wpdb->prefix . 'vbro_images WHERE native_url=\'' . $url . '\'';
        ...
        if (count($rows) != 0) {
            ...
            return vbro_images::create_image($url, $width, $height, $path);
        }
    }
    ...
    static public function create_image($url, $w = false, $h = false, $path = false)
    {
        ...
        require_once($dir . '/v_image_functions.php');
        require_once($dir . '/v_url_to_filepath.php');

        $dest_path = vusquare_functions::get_current_upload_dir();
        $suffix = "_resized";

        $file = vusquare_functions::get_filepath_from_url_smart($url);
        $img = vall_around_image_class::create_object($file);

        if ($img && !$img->is_error()) {
            if ($w && $h)
                $img->resize($w, $h, true);
            $file2 = $img->save($suffix, $dest_path);
            $url2 = vusquare_functions::get_full_urlpath_from_full_filepath($file2);
