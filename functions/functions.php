...
$wp_v_my_timeline_has_wmpl = false;
...
function wp_my_v_timeline_clone_timeline_settings($id)
{

}

function wp_v_my_timeline_get_translations($lang)
{
...
function wp_v_my_timeline_getCatsWpml($lang)
{
    global $wp_my_timeline_wpml_deafult_lang;
 ...
 * @param unknown $lang
 */
function wp_v_my_timeline_switch_to_lang($lang)
{
    if (class_exists('SitePress')) {
        global $sitepress;
        $sitepress->switch_lang($lang);
        wp_v_my_timeline_debug_title("Switrch to lang", $lang);
    }
}
 ...
 * Has wpml
 */
function wp_v_my_timeline_has_wmpl()
{
    global $wp_v_my_timeline_has_wmpl;
    global $wp_my_timeline_wpml_deafult_lang;

    if (class_exists('SitePress')) {
        $wp_v_my_timeline_has_wmpl = true;
        //echo 'Has wpml';
        global $sitepress;
        $def = $sitepress->get_default_language();
        //wp_v_my_timeline_debug_title("Def", $def);
 ...
 * Active languages
 */
function wp_v_my_timeline_active_languages()
{
    global $wp_v_my_timeline_has_wmpl;
    global $wp_my_timeline_wpml_languages;
    //wp_v_my_timeline_debug_title("Has wpml", $wp_v_my_timeline_has_wmpl);
...
function wp_v_my_timeline_debug_title($t, $o)
{
    ?>
...
function wp_v_my_timeline_get_last_item_key($items)
{
    $my_id = 0;
    $max = 0;
    if (is_array($items)) {
...
function wp_v_my_timeline_get_timestamp($y, $m, $d)
{
    $t = $d;
    $arr = array(
...
function wp_v_my_timeline_cat_years(&$items)
{
    foreach ($items as $key => $val) {
        $k = $key;
        ...
        $timestamp = wp_v_my_timeline_get_timestamp($year, $month, $day);
        $items[$k]['my_timestamp'] = $timestamp;
        $items[$k]['categoryid'] = 'timeline';
        $items[$k]['node-name'] = $year;
    }
    //usort($items, "wp_v_my_timeline_cmp");
}
 ...
 * @return number
 */
function wp_v_my_timeline_cmp($a, $b)
{
 ...
 *
 */
function wp_v_my_timeline_add_new_posts($id, $cats, &$items, $start_latest = false, $st_id)
{
    global $wp_my_timeline_limit_total_posts;
    $limit = -1;
    ...
    $my_debug = false;
    if ($my_debug) {
        wp_v_my_timeline_debug_title('Cats', $cats);

        wp_v_my_timeline_debug_title('Old items', $items);

        wp_v_my_timeline_debug_title('Cats posts', $cats_posts);
    }
    ...
    if ($my_debug) {
        wp_v_my_timeline_debug_title('New posts', $new_posts);
    }
    global $wpdb;
    $wpdb->show_errors();
    //$st_id+=1;
    if (!empty($new_posts)) {
        if (!$start_latest) {
            ...
            $vertical_timeline = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'vctimelines WHERE id=' . $id);
            $vertical_timeline = $vertical_timeline[0];
            $items_str = $vertical_timeline->items;
            ...
            foreach ($new_posts as $key => $val) {
                $items_str .= wp_v_my_timeline_format_items_to_db($val, $st_id);
                $st_id++;
                if ($my_c_12 < $t_t) $items_str .= '||';
                $my_c_12++;
            }
            $wpdb->update($wpdb->prefix . 'vctimelines', array('items' => $items_str), array('id' => $id));
            if ($my_debug) {
                wp_v_my_timeline_debug_title("Items str", array('id' => $id, 'items_str' => $items_str));
            }
        } else {
            $my_cats_ids = array();
            foreach ($cats as $key => $val) {
            ...
            foreach ($items as $key => $val) {
                ...
                $items_str .= wp_v_my_timeline_format_items_to_db($val, $st_pre);
                //$st_pre++;
                $items_str .= '||';
            }
            $t_t = count($new_posts) - 1;
            ...
            if ($my_debug) {
                wp_v_my_timeline_debug_title("Items str", array('id' => $id, 'start-item' => $my_start_item_id, 'items_str' => $items_str));
            }
            $wpdb->update($wpdb->prefix . 'vctimelines', array('items' => $items_str), array('id' => $id));
 ...
 * @return string
 */
function wp_v_my_timeline_format_items_to_db($arr, $id)
{
    $add_key = 'sort' . $id . '-';
    $str = '';
    ...
    //$str.='||';
    //wp_v_my_timeline_debug_title("Post data str", $str);
    return $str;
}
 ...
 * Get cats ids
 */
function wp_v_my_timeline_get_timeline_cats($settings)
{
    $ret = array();
 ...
 * @return string
 */
function wp_v_my_timeline_share_post($post_id, $post_title, $post_content, $image = '', $type = 'twitter')
{
    if (!empty($post_id)) {
    ...
    switch ($type) {
        ...
        case 'facebook':
            ...
            ?>
            https://www.facebook.com/sharer/sharer.php?u=&link=<?php echo urlencode($url) ?>&caption=<?php echo urlencode($post_title) ?>&description=<?php echo urlencode($text); ?>&picture=<?php echo urlencode($image); ?>
            <?php
            break;
        case 'google':
            ?>
 ...
 * @param unknown $post_id
 */
function wp_v_my_timeline_get_post_meta_data($post_id)
{
    $ret['hearts'] = 0;
    $ret['comments'] = 0;
    ...
    //$my_var=rand(1E3, 1E9);
    $ret['comments'] = wp_v_my_timeline_format_number($my_var);
    $my_var = get_post_meta($post_id, '_my_timeline_12_12_hearts', true);
    if (empty($my_var)) {
        $my_var = 0;
    }
    $ret['hearts'] = wp_v_my_timeline_format_number($my_var);
    return $ret;
}
 ...
 */
function wp_v_my_timeline_format_number($var)
{
    $check = array(
        'T' => 1E12,
        'G' => 1E9,
 ...
 * @return boolean
 */
function wp_v_timeline_get_post_id_by_title($title)
{
    global $wpdb;
 ...
 */
function wp_v_my_timeline_render_options($options, $title, $append = '')
{
    ...
    <div id="pbox<?php echo $wp_my_timeline_options_index ?>" class="postbox closed">
        ...
        <div class="my_blue_header my_no_margin">
            ...
            <div class="inside postbox-content">
                    ...
                    foreach ($options as $key => $val) {
                        ...
                        if ($val['type'] == 'image') {
                            ...
                            <tr class="field-row" id="my_option_<?php echo $key ?>">
                                <td colspan="2">
                                    <span class="timeline-help dashicons dashicons-editor-help"><span
                                                class="timeline-tooltip"><?php echo $val['tooltip']; ?></span></span>
                            ...
                            <tr class="field-row">
                                <td colspan="2">
                                    <div class="misc-pub-section timeline-pub-section">
                                        <?php echo wp_v_my_timeline_render_element($key, $val); ?>
                                    </div>
                                </td>
                            </tr>
                            <?php
                            ...
                            ?>
                            <tr class="field-row header-field-row">
                            <td colspan="2">
                                <h3 class="my_submenu_1"><?php echo $val['title']; ?></h3>
                            </td>
                            <?php
                        } else if ($val['type'] == 'one') {
                            ?>
                        ...
                        <?php } else { ?>
                            <?php if ($key == "style" || $key == "cat-type" || $key == "close-button-position" || $key == "category-alignment" || $key == "title-font-type" || $key == "subtitle-font-type" || $key == "excerpt-font-type" || $key == "fulltext-font-type" || $key == "date-font-type" || $key == "title-position" || $key == "subtitle-position" || $key == "title-alignment" || $key == "subtitle-alignment" || $key == "excerpt-alignment" || $key == "fulltext-alignment" || $key == "date-alignment" || $key == "category-position" || $key == "category-type" || $key == "node-shape" || $key == "node-text" || $key == "close-button" || $key == "show-date" || $key == "tim-align" || $key == "image-position" || $key == "image-shape" || $key == "read-more" || $key == "search-bar-position" || $key == "align-start-item" || $key == "hide-show-search" || $key == "line-style" || $key == "nav-style" || $key == "shadow" || $key == "easing" || $key == "button-type" || $key == "item-header-font-type" || $key == "item-text-font-type" || $key == "item-text-align" || $key == "item-open-header-font-type" || $key == "item-open-text-font-type") { ?>
                                <tr class="field-row" id="my_option_<?php echo $key ?>">
 ...
 * @return string
 */
function wp_v_my_timeline_render_element($name, $element)
{
    $type = $element['type'];
    ...
    switch ($type) {
        case 'image':
            ...
            else $image = $wp_my_timeline_url . '/images/no_image_background.jpg';
            ?>
            <div class="cw-image-select-holder">
                <input id="<?php echo esc_attr($name) . '-input' ?>" name="<?php echo esc_attr($name) ?>" type="hidden"
                       value="<?php if (isset($value)) echo $value; ?>"/>
                <a href="#" id="<?php echo esc_attr($name); ?>" class="cw-image-upload"
                   style="<?php echo 'background: url(' . $image . ');'; ?>"></a>
                <small><?php echo __("Click on image to change, or"); ?> <a href="#" class="remove-image">remove
                        image</a></small>
            </div>
        ...
        case 'color':
            ?>
            <input class="timeline_color_input" id="<?php echo esc_attr($name) ?>" name="<?php echo esc_attr($name) ?>"
                   value="<?php if (isset($value)) echo $value; ?>" type="text"
                   style="background:#<?php if (isset($value)) echo $value; ?>;">

            <?php
            break;
        case 'text':
            ?>
            <input type="text" value="<?php if (isset($value)) echo esc_attr($value); ?>"
                   name="<?php echo esc_attr($name); ?>" id="<?php echo esc_attr($name) ?>"
                   size="<?php /* if(isset($element['size']))echo esc_attr($element['size']);*/
                   ?>3"/>
        ...
        case 'checkbox':
            ?>
            <input type="checkbox" <?php if (!empty($value)) echo 'checked="checked"' ?>
                   value="<?php if (isset($element['val'])) echo esc_attr($element['val']); else echo '1'; ?>"
                   name="<?php echo esc_attr($name); ?>" id="<?php echo esc_attr($name) ?>"/>
            <?php
            break;
        case 'select':
            ...
            <select name="<?php echo esc_attr($name); ?>" id="<?php echo esc_attr($name) ?>">
                <?php if (!empty($element['values'])) { ?>
                    <?php foreach ($element['values'] as $k => $v) { ?>
                        <option <?php if (isset($value) && $value == $k) echo 'selected="selected"' ?>
                                value="<?php echo esc_attr($k) ?>"><?php echo $v ?></option>
                    <?php } ?>
                <?php } ?>
            </select>
            <?php
