...
if (!empty($wp_v_my_timeline_has_wmpl)) {
    ...
    ?>
    <div class="wrap">
        <h2><?php echo __("Timeline Tranlsations ", 'my_vertical_content_timeline'); ?></h2>
        ...
        <?php if (!empty($wp_v_my_timeline_has_wmpl)) {
                    ...
                    foreach ($wp_my_timeline_wpml_languages as $k123 => $v123) {
                        ...
                        <li>
                            <?php if ($v123['code'] != $my_lang) { ?>
                                <a href="<?php echo admin_url("admin.php?page=verticalcontenttimeline_translations&lang=" . $v123['code']); ?>"><?php echo $v123['display_name']; ?></a>
                            <?php } else { ?>
                                <span><?php echo __("Translations : ", 'my_vertical_content_timeline') . $v123['display_name']; ?></span>
                            <?php } ?>
                        </li>
                        <?php
                    }
            ...
            <?php if ($my_lang == 'en') { ?>
                ...
                <h4 style="color:red"><?php echo __("Please select above new language to add translations ", 'my_vertical_content_timeline') ?></h4>
            <?php } else { ?>
                <form method="post">
                    <ul>
                        <li>
                            <input type="submit" class="button button-large button-primary" name="my_submit"
                                   value="<?php echo __("Save Translation", 'my_vertical_content_timeline') ?>"/>
                        </li>
                        ...
                        <?php foreach ($wp_my_timeline_translations as $k => $v) { ?>
                            ...
                            if ($k == 'months') {
                                ?>
                                <li>
                                    <label><?php echo __("Months Translations : ", 'my_vertical_content_timeline') ?></label>
                                </li>
                                <?php
                            }
                            if (is_array($v)) {
                                foreach ($v as $k1 => $v1) {
                                    ...
                                    <li><label><?php echo $v1; ?></label>
                                        <input type="text" value="<?php echo $options[$my_lang]['months'][$k1] ?>"
                                               name="<?php echo $k . '_' . $k1 ?>"/>
                                    </li>
                                    <?php
                                }
                            }
    ...
    <form method="post">
        <ul>
            ...
            <?php foreach ($wp_my_timeline_translations as $k => $v) { ?>
                ...
                if ($k == 'months') {
                    ?>
                    <li><label><?php echo __("Months Translations : ", 'my_vertical_content_timeline') ?></label></li>
                    <?php
                }
                if (is_array($v)) {
                    foreach ($v as $k1 => $v1) {
