...
if ($settings['cat-type'] == 'months') {
    ...
    if ($my_lang != 'en') {
        ...
        $t = wp_v_my_timeline_get_translations('en');
        //print_r($t);
        $myCats1 = '';
...
$(window).load(function() {
	if(!test)
		vertical_timeline_init_'.$id.'($(document));
});';
$frontHtml .= '
	function vertical_timeline_init_' . $id . '($this) {
		$this.find(".scrollable-content").mCustomScrollbar();' . $my_pretty_photo . '
				$this.find("#vtl' . $id . '").VerticalTimeline({';
if (isset($myCats)) {
...
$frontHtml .= 'my_show_years:' . $settings['num-of-years'] . ',
			...
			search_bar_position : ' . $search_bar_position . ',
    ...
    $cats . ',
		...
		$this.find("#vtl' . $id . '").on("ajaxLoaded.timeline", function(e){
			...
			' . $my_pretty_photo_1 . '
					e.element.find(".vertical_timeline_rollover_bottom").timelineRollover("bottom");
		});
	}
	})(jQuery);
	</script>';
