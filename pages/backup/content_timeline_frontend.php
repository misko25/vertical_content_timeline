...
#tl' . $id . ' .vertical_t_line_view,
#content #tl' . $id . ' .vertical_t_line_view {
 	width: ' . $settings['line-width'] . 'px;
} 
...
#tl' . $id . ' .item .vertical_con_borderImage,
#content #tl' . $id . ' .item .vertical_con_borderImage {
 	border:0px;
 	border-bottom: ' . $settings['item-image-border-width'] . 'px solid ' . $settings['item-image-border-color'] . ' ; 
}
 
#tl' . $id . ' .item_open .vertical_con_borderImage,
#content #tl' . $id . ' .item_open .vertical_con_borderImage {
...
#tl' . $id . ' .vertical_t_node_desc,
#content #tl' . $id . ' .vertical_t_node_desc,
 {
...
#tl' . $id . ' .vertical_timeline_open_content,
#content #tl' . $id . ' .vertical_timeline_open_content {
...
if ($vertical_timeline->items != '') {
    ...
    foreach ($itemsArray as $key => $arr) {
		...
		<div class="item" data-id="' . $arr['dataid'] . '" data-description="' . substr($arr['item-title'], 0, 30) . '">
			<img class="vertical_con_borderImage" src="' . $this->url . 'timthumb/timthumb.php?src=' . $arr['item-image'] . '&w=' . ((int)$settings['item-width'] - 10) . '&h=' . ((int)$settings['item-image-height'] - 10) . '" alt=""/>
        ...
        if ($arr['item-open-image'] != '') {
            $frontHtml .= '
			<img class="vertical_con_borderImage" src="' . $this->url . 'timthumb/timthumb.php?src=' . $arr['item-open-image'] . '&w=' . ((int)$settings['item-open-width'] - 10) . '&h=' . ((int)$settings['item-open-image-height'] - 10) . '" alt=""/>
			<div class="vertical_timeline_open_content' . (!$arr['desable-scroll'] ? ' scrollable-content' : '') . '" style="height: ' . $open_content_height . 'px">';

        } else {
            $frontHtml .= '
			<div class="vertical_timeline_open_content' . (!$arr['desable-scroll'] ? ' scrollable-content' : '') . '" style="height: ' . (intval($settings['item-height']) - 2 * intval($settings['item-open-content-padding'])) . 'px">';
        }
