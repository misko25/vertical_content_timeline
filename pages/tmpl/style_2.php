...
if (!empty($arr['my-post-id'])) {
    ...
    $my_post_id = wp_v_timeline_get_post_id_by_title($arr['item-title']);
    if (!empty($my_post_id)) {
        $my_has_post_id = true;
    }
}
$my_metadata = wp_v_my_timeline_get_post_meta_data($my_post_id);
?>
<div class="item"
...
echo $my_name; ?> data-description="<?php echo mb_substr($arr['item-title'], 0, 30); ?>">
    ...
    <div class="my_vertical_timeline_content">
        ...
        <?php } else { ?>
            ...
            if ($my_post_link_val == 1 && !empty($my_has_post_id)) {
                ...
                <h2 class="my_read_more" data-href="<?php echo esc_attr($my_link_12_1234); ?>"
                    data-id="<?php echo $arr['dataid'] ?>"><?php echo $arr['item-title']; ?></h2>
                <?php
            } else {
                ?>
        ...
        <div class="my_share_items  my_clear">
            ...
            <?php if (!empty($arr['my-post-id'])) { ?>
                ...
                <div class="my_float_right my_align_right <?php if (!$settings['show-date']) echo 'my_share_no_date' ?> my_share_items_1">
                    <div class="my_share_item my_like_post" data-liked="0"
                         data-id="<?php echo $id . '_' . $iteration ?>"
                         data-post-id="<?php if ($my_has_post_id) echo $my_post_id ?>"
                         data-val="<?php echo $my_metadata['hearts'] ?>">
    ...
    <div class="item_open <?php ($arr['item-image'] == '' ? ' item_open_noImg' : ''); ?>"
         data-id="<?php echo $arr['dataid']; ?>" <?php $var = (!isset($tpreview) ? 'data-access="' . admin_url('admin-ajax.php') . '?action=vctimeline_frontend_get&vtimeline=' . $id . '&id=' . $key . '"' : '');
    echo $var; ?>>
